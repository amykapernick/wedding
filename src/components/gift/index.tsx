...
const Gift = ({ properties, id }: NotionGift) => {
	...
	return (
		<>
			...
			<li className={styles.gift} data-price={properties["Price"].select?.name} data-purchased={purchased} data-quantity={properties["Quantity"].number} data-left={properties["Quantity"].number - purchased}>
				<h2 className={styles.name}>
					<a href={properties["URL"].url} target="_blank">
				...
				{properties["Image"]?.files?.[0]?.file && (
					<div className={styles.image}>
						<Image alt="" src={properties["Image"].files[0].file.url} width="300" height="300" />
					</div>
				)}
				{id && (
					<form action={claimGift} className={styles.button}>
						<button type="submit" disabled={purchased >= properties["Quantity"].number} aria-disabled={purchased >= properties["Quantity"].number}>
							{buttonText}
						</button>
					</form>
				)}
